<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="specsname" label="名称"> </el-table-column>
      <el-table-column prop="attrs" label="属性"> 
        <template #default="props">
          <el-tag type="info" v-for="item of props.row.attrs" :key="item" style="margin-right:5px">{{item}}</el-tag>
        </template>
      </el-table-column>
...
export default {
  computed: {
    ...mapState({
      ...
      size: state => state.specs.size, //当前管理员列表数据
      page: state => state.specs.page, //每页显示的数量
